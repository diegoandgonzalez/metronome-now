...
const isKeyPresentInLocalStorage = (key: string) =>
  isLocalStorageAvailable() && Boolean(localStorage.getItem(key));
...
export const setValueInLocalStorage = (key: string, value: LocalStorageValueType) => {
  if (!isLocalStorageAvailable()) return;
